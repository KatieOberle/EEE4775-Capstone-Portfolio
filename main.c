...
static void btn_task_sem(void *arg)
{
    for (;;) {
        if (xSemaphoreTake(btn_sem, portMAX_DELAY) == pdTRUE) {
             ...
             */
            ESP_LOGI(TAG, "[sem] Security Tamper event #%lu, integrity flag raised latency=%lld us (max=%llu)",
...
static void btn_task_notif(void *arg)
{
    for (;;) {
        ...
        ESP_LOGI(TAG, "[notif] Security Tamper event #%lu recorderd, alert sent latency=%lld us (max=%llu) "
                      "(notif count=%lu)",
                 (unsigned long)presses_observed,
                 (long long)lat,
